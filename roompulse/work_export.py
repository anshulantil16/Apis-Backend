...
SUMMARY_COLUMNS = [
    ...
    ('Outside office hours', 20),
]
DETAIL_COLUMNS = [
    ('Date', 12), ('Person', 24), ('How it came in', 16), ('Category', 26),
    ('What was done', 46), ('For', 24), ('Worked', 15), ('Time', 10),
    ('Outside hours', 14), ('State', 14),
]
...
def build_work_report(report, tickets):
    ...
    ws['A4'] = (f"Counted by the day the work was done. A ticket raised last month and "
                ...
                f"{report.get('office_hours', '09:30-18:30')}; anything outside them, and "
                f"anything at all on a Saturday or Sunday, is counted as outside hours.")
    ...
    for p in report['people']:
        _row(ws, r, [p['name'], p['email'], p['closed'], p['logged'],
                     ...
                     _minutes(p.get('after_hours_minutes'))])
        r += 1
